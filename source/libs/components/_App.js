  ...
  >
    <div className={
      cx({
        ...
        "vandal-tooltip__date--empty": !props.tooltip.ts || !props.tooltip.ts.length
      })
    }>
    ...
    <div className="vandal-tooltip__ts">
      {
        props.tooltip.ts && props.tooltip.ts.map((ts, i) => {
          return <div className="vandal-tooltip__ts__value" key={`ts-${i}`}>{toTwelveHourTime(ts.toString().substr(-6))}</div>
        })
      }
    </div>
  </div>
...
export default class Vandal extends React.Component {
  ...
  showYear(year) {
    return async () => {
      const url = `https://web.archive.org/__wb/calendarcaptures?url=${this.url}&selected_year=${year}`
      const calendarData = await api(url);
      const filteredMonths = calendarData.map((month) => (flatten(month).filter(Boolean)));
      ...
      this.setState({months: aggregatedMonths, selectedYear: year, maxCountInYear});
    }
  }
  ...
  render() {
    let { years, months, selectedYear, maxCountInYear, x, y, showTooltip, tooltip, cx, cy } = this.state;
        ...
        <div className="vandal-year_container">
          {
            newYearsArr.map((y, i) => {
              return (
                <div className="vandal-year" key={`year-${y}`} onClick={this.showYear(y)}>
                  <span className="vandal-year__value">{y}</span>
                  <Sparklines className="vandal-year__sparkline" data={years[y]} margin={0} width={48} height={height} min={0} max={maxcount}>
                    <SparklinesBars/>
                  </Sparklines>
                </div>
              );
        ...
        <div className="vandal-month_container">
          {
            months.map((m, index) => {
              ...
              const points = dataToPoints({ data: countArr, limit: 0, width: 80, height: 20, margin: 0, min: 0, max: !max ? max + 1 : max });

              return (
                <div className="vandal-month" key={`month-${index}-${selectedYear}`}>
          ...
          {showTooltip && <Tooltip x={x} y={y} tooltip={tooltip} selectedYear={selectedYear} onMouseEnter={this.onTooltipEnter} onMouseMove={this.onTooltipEnter} onMouseLeave={this.onTooltipLeave}/>}
        </div>
      </div>
    );
  };
  ...
  async componentDidMount() {
    ...
    const sparklineData = await api(`https://web.archive.org/__wb/sparkline?url=${this.url}&collection=web&output=json`);
